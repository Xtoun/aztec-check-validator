...
CF_CURL_BIN=""
detect_cf_curl() {
  # попробуем curl-impersonate (если установлен)
...
cf_random_ua() {
  # лёгкая рандомизация минорной версии UA
  local major="${1:-139}"
...
# Базовые заголовки, похожие на браузерные
_cf_hdrs_common() {
...
# cf_curl: “максимально похожий на браузер” запрос
cf_curl() {
...
check_validator_queue() {
    ...
    check_single_validator() {
        ...
        # Используем CF-дружественный запрос с Origin/Referer
        local response_data
...
# === CF-friendly curl (embedded) ===
CF_CURL_BIN=""
if command -v curl_chrome141 >/dev/null 2>&1; then CF_CURL_BIN="curl_chrome141";
elif command -v curl_chrome140 >/dev/null 2>&1; then CF_CURL_BIN="curl_chrome140";
elif command -v curl_chrome139 >/dev/null 2>&1; then CF_CURL_BIN="curl_chrome139";
else CF_CURL_BIN="curl"; fi
...
cf_curl_json() {
  ...
  local resp http
  if [ "$CF_CURL_BIN" != "curl" ]; then
    resp="$("$CF_CURL_BIN" --http2 --compressed --location --silent --show-error \
      $(_cf_hdrs_common) "$@" "$url" -w $'__HTTP:%{http_code}')" || true
  else
    resp="$(curl --http2 --tlsv1.3 --compressed --location --silent --show-error \
...
# Configuration will be injected by parent script via env export before heredoc ends
...
monitor_position() {
    ...
    response_data="$(cf_curl_json "$search_url")"
    if [ -z "$response_data" ]; then
        log_message "Error: Failed to fetch queue data (CF)"
        return 1
    fi
...
# Конец: переменные подставляются родительским скриптом через envsubst-замены ниже
EOF

        # Подставим реальные значения в heredoc
        sed -i \
